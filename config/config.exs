# This file is responsible for configuring your application
# and its dependencies with the aid of the Mix.Config module.
#
# This configuration file is loaded before any dependency and
# is restricted to this project.

# General application configuration
use Mix.Config

config :hyper_schedule, :children, [
  # Start the Ecto repository
  #      HyperSchedule.Repo,
  # Start the Telemetry supervisor
  HyperScheduleWeb.Telemetry,
  # Start the PubSub system

  {Phoenix.PubSub, name: HyperSchedule.PubSub},
  # Start the Endpoint (http/https)
  HyperScheduleWeb.Endpoint
  # Start a worker by calling: HyperSchedule.Worker.start_link(arg)
  # {HyperSchedule.Worker, arg}
]

config :hyper_schedule,
  ecto_repos: [HyperSchedule.Repo]

# Configures the endpoint
config :hyper_schedule, HyperScheduleWeb.Endpoint,
  url: [host: "localhost"],
  secret_key_base: "7EVTuIwqJGuU6Z9UXwITpyh8ams/51bRenLsg8xRiMtVwJkHG3p1wCpnWut+s4Y2",
  render_errors: [view: HyperScheduleWeb.ErrorView, accepts: ~w(html json), layout: false],
  pubsub_server: HyperSchedule.PubSub,
  live_view: [signing_salt: "KxkgIFJ3"]

# Configures Elixir's Logger
config :logger, :console,
  format: "$time $metadata[$level] $message\n",
  metadata: [:request_id]

# Use Jason for JSON parsing in Phoenix
config :phoenix, :json_library, Jason

# Configure esbuild (the version is required)
config :esbuild,
  version: "0.12.18",
  default: [
    args: ~w(js/app.js --bundle --target=es2016 --outdir=../priv/static/assets),
    cd: Path.expand("../assets", __DIR__),
    env: %{"NODE_PATH" => Path.expand("../deps", __DIR__)}
  ]

# Import environment specific config. This must remain at the bottom
# of this file so it overrides the configuration defined above.
import_config "#{Mix.env()}.exs"
